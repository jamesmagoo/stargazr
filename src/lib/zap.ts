...
export const zapNote = async (note: any, sender: string | undefined, amount: number, comment = '', relays: Relay[]) => {
  if (!sender) {
    return false;
  }
...
export const getZapEndpoint = async (user: any): Promise<string | null>  => {
  try {
    let lnurl: string = ''
...
export const canUserReceiveZaps = (user: any | undefined) => {
